...
class MakeBakeUpPopupWindow(mContext: Context) : PopupWindow(), View.OnClickListener {
    var view: View
    var btnOverFile: Button
    var btnKeepFile: Button
    var presenter = MakeBackUpPresenter(mContext)

    init {
        view = LayoutInflater.from(mContext).inflate(R.layout.popup_window_make_backup, null)
        btnOverFile = view.findViewById(R.id.save_local)
        btnKeepFile = view.findViewById(R.id.save_drive)
        // 外部可點擊
