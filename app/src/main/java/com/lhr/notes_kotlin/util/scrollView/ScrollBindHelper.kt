...
import com.lhr.notes_kotlin.util.VerticalSeekBar

class ScrollBindHelper(
    private val seekBar: VerticalSeekBar,
    private val scrollView: ScrollView
) :
    SeekBar.OnSeekBarChangeListener, ObservableScrollView.ScrollViewListener {
    private val scrollContent: View
    private var isUserSeeking = false
    private val contentRange: Int
        private get() {
            seekBar.max = scrollContent.height - scrollView.height
            return scrollView.scrollY
        }
    private val scrollRange: Int
        private get() {
            println(scrollContent.height - scrollView.height)
            return scrollContent.height - scrollView.height
        }
